...
class FollowerWorker(QThread):
    ...
    def run(self):
        try:
            graphql_endpoint = Config.FOLLOWER_URL + '&variables={}'
            ...
            while has_next_data:
                ...
                data = json.loads(pre)['data']['user']['edge_followed_by']
                ...
                for user in edges:
                    try:
                        ...
                        if rownum % 100 == 0:
                            Common.debug(self.parent.log_browser, f"팔로워 추출 진행 중. 건 수 : {str(rownum)}")
            ...
            Common.error(self.parent.log_browser, f"Follower Run 실행 중 오류 메시지 : {str(e)}")
        finally:
            self.parent.callback('follower')